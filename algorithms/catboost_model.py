...
class DemandPredictor:
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Feature isimlerini restore et (load sonrası eksik kalabilir)
        if self.model and not self._feature_names:
            try:
                self._feature_names = self.model.feature_names_
                self.is_trained = True
            except Exception:
                pass
    ...
    def load(self, version=1):
        """Model dosyasını yükler ve _feature_names'i restore eder."""
        path = os.path.join(self.model_dir, f"catboost_v{version}.cbm")
        if os.path.exists(path):
            self.model = CatBoostRegressor()
            self.model.load_model(path)
            self.is_trained = True
            # Feature isimlerini CatBoost modelinden restore et
            try:
                self._feature_names = self.model.feature_names_
            except Exception:
                self._feature_names = []
