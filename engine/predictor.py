...
class PredictionEngine:
    ...
    def train(self, df: pd.DataFrame, verbose=True):
        ...
        # Kolon adı normalizasyonu: DB → engine beklentisi
        col_renames = {}
        if "product_category" in df.columns and "category" not in df.columns:
            col_renames["product_category"] = "category"
        elif "product_category" in df.columns and df["category"].isna().all():
        ...
        if verbose:
            print("[Katman 4a] CatBoost v2 — per-category eğitim")

        cat_results = self.registry.train_all(df_feat, verbose=verbose)
        trained_count = sum(1 for v in cat_results.values() if v)

        # Eğitilen kategori yoksa global fallback
        if trained_count == 0:
            if verbose:
                print("  ⚠ Hiçbir kategori eğitilemedi — global fallback")
            ...
            if verbose:
                print(f"  ✓ {trained_count}/{len(cat_results)} kategori eğitildi")
        if verbose:
            print()
    ...
    def predict_for_inventory(self, material: str, color: str = None,
        ...
        for cat in categories:
            ...
            # Per-category model varsa onu kullan, yoksa global fallback
            cat_model = self.registry.get_or_create(cat)
            predictor = cat_model if cat_model.is_trained else self.catboost
            raw_score = predictor.predict_for_representative(
    ...
    def status(self) -> dict:
        """v3: Zengin motor durumu — per-category registry dahil."""
